...
typedef struct csky_arch_for_merge
{
  ...
  /* The files can merge only if the are in same class.  */
  enum merge_class class;
  /* When input files has different levels,
     the target set arch_eflag to the largest level file's arch_eflag.  */
  unsigned int class_level;
  /* Control whether print warning when merge with different arch.  */
...
static struct csky_arch_for_merge csky_archs[] =
{
  ...
  { "ck804",  CSKY_ARCH_804,  CSKY_V2,  3, 1},
  { "ck805",  CSKY_ARCH_805,  CSKY_V2,  4, 1},
  { "ck807",  CSKY_ARCH_807,  CSKY_V2,  5, 1},
  { "ck810",  CSKY_ARCH_810,  CSKY_V2,  6, 1},
  { "ck860",  CSKY_ARCH_810,  CSKY_V2,  7, 1},
  { "ck800",  CSKY_ARCH_800,  CSKY_V2,  8, 1},
  { NULL, 0, 0, 0, 0}
};
                            ...
                            bfd_reloc_code_real_type code)
{
  ...
  /* Note: when add csky bfd reloc types in bfd-in2.h
...
/* In check_relocs, we keep track of the number of relocs that it decides
   to copy as dynamic relocs  for each symbol. So that it can later
   discard them if they are found to be unnecessary. We store the
   information in a field extending the regular ELF linker hash table.  */
...
#define csky_elf_hash_table(info) \
  ...
  == CSKY_ELF_DATA ? ((struct csky_elf_link_hash_table *) ((info)->hash)) : NULL)
...
struct csky_elf_link_hash_table
{
  ...
  /* Linker call-backs.  */
  /*FIXME: maybe have some change in add_stub_section parmas.  */
  ...
  /* Number of elemernts in stub_group.  */
  unsigned int top_id;
...
/* Function declare.  */
static bfd_vma
_csky_get_insn_32 (bfd *input_bfd,
                   bfd_byte *location);

static void
_csky_put_insn_32 (bfd *input_bfd, bfd_vma x,
                   bfd_byte *location);
                   ...
                   const char *name ATTRIBUTE_UNUSED)
{
  ...
  if (r_type == R_CKCORE_PCREL_IMM26BY2
    ...
    {
      if (((elf_elfheader (info->output_bfd)->e_flags & CSKY_ARCH_MASK)
           ==  CSKY_ARCH_810)
          || ((elf_elfheader (info->output_bfd)->e_flags & CSKY_ARCH_MASK)
              ==  CSKY_ARCH_807))
        {
...
/* Create .plt, .rel.plt, .got, .got.plt, .rel.got, .dynbss, and
   .rel.bss sections in DYNOBJ.
   FIXME: maybe it depend on whether we need stortcuts in other functions
   or not, that should we set up shortcuts to them in our hash table.  */

static bfd_boolean
csky_elf_create_dynamic_sections (bfd *dynobj,
                                  struct bfd_link_info *info)
{
  struct csky_elf_link_hash_table *htab;
                                ...
                                struct elf_link_hash_entry *h)
{
  ...
  /* If allowed, we must generate a R_CKCORE_COPY reloc to tell the dynamic
...
static bfd_boolean
_csky_allocate_dynrelocs (struct elf_link_hash_entry *h, PTR inf)
{
  struct bfd_link_info *info;
  struct csky_elf_link_hash_table *htab;
  struct csky_elf_link_hash_entry *eh;
      ...
      if (bfd_link_pic (info) || WILL_CALL_FINISH_DYNAMIC_SYMBOL (1, 0, h))
        {
          ...
          if (splt->size == 0)
            {
              if ((elf_elfheader (info->output_bfd)->e_flags& CSKY_ABI_MASK)
                  ==  CSKY_ABI_V1)
...
static bfd_boolean
_csky_readonly_dynrelocs (struct elf_link_hash_entry *h, PTR inf)
{
  struct csky_elf_link_hash_entry *eh;
  struct csky_elf_dyn_relocs *p;
      ...
      if (s != NULL && (s->flags & SEC_READONLY) != 0)
        {
          ...
          /*DF_TEXTREL flag means there are at least one reloction in
            read-only sections, it may modifys those sections
            when relocating.  */
                                ...
                                struct bfd_link_info *info)
{
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link.next)
    {
      ...
      for (; local_got_refcounts < end_local_got; ++local_got_refcounts,++local_tls_type)
        {
  ...
  /* Allocate global sym .plt and .got entries, and space for global
     sym dynamic relocs.  */
  elf_link_hash_traverse (&htab->elf, _csky_allocate_dynrelocs, (PTR) info);
  /* Judge .got .got.plt size.  */
  if(got_size_flags == 1)
    {
      if((htab->elf.sgot->size + htab->elf.sgotplt->size) > GOT_MAX_SIZE )
        {
          _bfd_error_handler (_("error: GOT TABLE SIZE OUT OF RAGNG!"));
          return FALSE;
        }
    }
      ...
      if (relocs)
        {
          ...
          if ((info->flags & DF_TEXTREL) == 0)
            elf_link_hash_traverse (&htab->elf, _csky_readonly_dynrelocs,
                                    (PTR) info);
  ...
  if(h->plt.offset != (bfd_vma) -1)
    {
      ...
      if ((h->dynindx == -1
          ...
          || relplt == NULL)
        abort ();
      if(((elf_elfheader (output_bfd)->e_flags)& CSKY_ABI_MASK) == CSKY_ABI_V2)
        {
      ...
      if(((elf_elfheader (output_bfd)->e_flags)& CSKY_ABI_MASK) == CSKY_ABI_V2 )
        {
          _csky_put_insn_32 (output_bfd, csky_elf_plt_entry_v2[0],
                             plt->contents + h->plt.offset);
          _csky_put_insn_32 (output_bfd, (csky_elf_plt_entry_v2[1] | plt_index),
                             plt->contents + h->plt.offset+4);
          _csky_put_insn_32 (output_bfd, csky_elf_plt_entry_v2[2],
...
static bfd_boolean
csky_elf_gc_sweep_hook (bfd * abfd ATTRIBUTE_UNUSED,
          ...
          case R_CKCORE_TLS_GD32:
            {
              ...
              if (r_type == R_CKCORE_GOT_IMM18BY4
                  && h != NULL
                  && bfd_link_executable (info)
                  && h->type == STT_OBJECT
                  && (sec->flags & SEC_ALLOC) != 0
              ...
              switch( ELF32_R_TYPE (rel->r_info))
                {
                  case R_CKCORE_TLS_IE32: tls_type = GOT_TLS_IE;break;
                  case R_CKCORE_TLS_GD32: tls_type = GOT_TLS_GD;break;
...
_csky_find_arch_with_eflag (const unsigned long arch_eflag)
{
  ...
  if (csky_arch == NULL)
    {
      msg = _("warning: unrecognised arch eflag '%#x'");
      (*_bfd_error_handler) (msg, arch_eflag);
      bfd_set_error (bfd_error_wrong_format);
    }

  if (csky_arch->arch_eflag == 0)
    return NULL;
...
elf32_csky_merge_attributes (bfd *ibfd, bfd *obfd)
{
  ...
  /* Skip any input that hasn't attribute section.
     ...
     any others.  */
  if (sec_name == NULL || bfd_get_section_by_name (ibfd, sec_name) == NULL)
    {
      return TRUE;
    }
  ...
  /* Check Floating 3.0 & (VDSP2.0 and Floating 3.0).  */
  if ( out_attr[Tag_CSKY_FPU_VERSION].i == VAL_CSKY_FPU_VERSION_3
       && in_attr[Tag_CSKY_FPU_VERSION].i == VAL_CSKY_FPU_VERSION_3)
    {
      if (out_attr[Tag_CSKY_VDSP_VERSION].i != 0
          && in_attr[Tag_CSKY_VDSP_VERSION].i != 0
          && (out_attr[Tag_CSKY_VDSP_VERSION].i
              != in_attr[Tag_CSKY_VDSP_VERSION].i))
        {
          msg = _("Error: file %B's VFPU feature is conflict "
                  "with file %B.");
      ...
      switch (i)
              ...
              if (new_arch->class != old_arch->class) {
              ...
              else if (new_arch->class_level != old_arch->class_level)
                {
                  ...
                  if (new_arch->do_warning || old_arch->do_warning)
                    {
                      msg = _("warning: file %B's arch flag %s conflict "
                              "with target %s,set target arch flag to %s ");

                       (*_bfd_error_handler) (msg, ibfd,  in_attr[i].s,
                                              out_attr[i].s,
                                              (attr[i].s));
                       bfd_set_error (bfd_error_wrong_format);
                    }
                      ...
                      if (out_attr[Tag_CSKY_ARCH_NAME].s != NULL)
                        {
                          out_attr[Tag_CSKY_ARCH_NAME].s =
                           _bfd_elf_attr_strdup (obfd,
                                                 attr[Tag_CSKY_ARCH_NAME].s);
                        }
                      if (out_attr[Tag_CSKY_CPU_NAME].s != NULL)
                        {
                          out_attr[Tag_CSKY_CPU_NAME].s =
                           _bfd_elf_attr_strdup (obfd,
                                                 attr[Tag_CSKY_CPU_NAME].s);
                        }
                    }
                }
            }
          else if (old_arch == NULL
                   && in_attr[Tag_CSKY_ARCH_NAME].s
                   && in_attr[Tag_CSKY_CPU_NAME].s)
            {
              out_attr[Tag_CSKY_ARCH_NAME].s =
               _bfd_elf_attr_strdup (obfd, in_attr[Tag_CSKY_ARCH_NAME].s);

              out_attr[Tag_CSKY_CPU_NAME].s =
               _bfd_elf_attr_strdup (obfd, in_attr[Tag_CSKY_CPU_NAME].s);
            }
        ...
        case Tag_CSKY_FPU_ABI:
          if (out_attr[i].i == 0
            ...
            {
              /* out float-abi is none 
                 out float-abi is SOFT and in is SOFTFP. */
              out_attr[i].i = in_attr[i].i;
            }
          else if (in_attr[i].i != 0
                   && out_attr[i].i != in_attr[i].i
                   && (out_attr[i].i == VAL_CSKY_FPU_ABI_HARD
                       || in_attr[i].i == VAL_CSKY_FPU_ABI_HARD))
            {
              _bfd_error_handler
               (_("Error: %B and %B has different FPU ABI"), ibfd, obfd);
               result = FALSE;
            }
          break;
...
/* If the symbol referenced by bsr is defined in shared object file,
   or it is weak symbol and we aim to create shared object file,
   create stub for this bsr and emit relocation for address data in
   stub when build stub. */
                  ...
                  do
                    {
                      ...
                      /* We've either created a stub for this reloc already,
                         or we are about to.  */
                      stub_entry = csky_stub_hash_lookup
      ...
      switch (template_sequence[i].type)
        {
          case INSN16:
            bfd_put_16 (stub_bfd,
                        (bfd_vma) template_sequence[i].data,
                        loc + size);
            size += 2;
            break;
          case INSN32:
            _csky_put_insn_32 (stub_bfd,
                               (bfd_vma) template_sequence[i].data,
                               loc + size);
            size += 4;
            break;
...
static void
_csky_put_insn_32 (bfd *input_bfd,
                   bfd_vma x,
                   bfd_byte *location)
{
    if(bfd_big_endian(input_bfd))
...
_csky_relocate_contents (reloc_howto_type *howto,
                         bfd *input_bfd,
                         long relocation,
                         bfd_byte *location)
{
...
#define N_ONES(n)      (((((bfd_vma) 1 << ((n) - 1)) - 1) << 1) | 1)
  ...
  /* 66574249, current maybe not aligned with rightshift.
   * make sure (pc + `relocation`) & mask aligned with rightshift .  */
  if (rightshift > 0)
    relocation += (1 << (rightshift - 1));
  ...
  if (((howto->type == R_CKCORE_DOFFSET_LO16)
  ...
  else
    {
      ...
      if ((howto->type == R_CKCORE_PCREL_IMM11BY2)
        ...
        {
          relocation = ((relocation<<4) & 0xf0)+((relocation<<17) & 0x1e00000);
        }
      else if (howto->type == R_CKCORE_NOJSRI)
        {
          x = (x & howto->dst_mask) | CSKY_INSN_JSRI_TO_LRW;
          relocation = 0;
          _csky_put_insn_32(input_bfd, CSKY_INSN_JSR_R26, location+4);
        }
...
{
  struct elf32_csky_stub_hash_entry *stub_entry;
  struct csky_elf_link_hash_entry *h = (struct csky_elf_link_hash_entry *) hash;
  const asection *id_sec;
...
static bfd_reloc_status_type
_csky_final_link_relocate (reloc_howto_type *howto,
                           bfd *input_bfd,
                           ...
                           bfd_vma addend)
{
  ...
  /* If the relocation is PC relative, we want to set RELOCATION to
     the distance beween the symbol (currently inRELOCATION) and the
  ...
  for (;rel < relend; rel++)
    {
      ...
      if ((unsigned) r_type >= (unsigned) R_CKCORE_MAX)
        {
          ...
          _bfd_error_handler (_("%B: Unknown relocation type %ud\n"),
      ...
      /* We don't need to handler R_CKCORE_NONE here. It's just a marker.  */
      else if (howto->type == R_CKCORE_NONE)
        continue;
      else
        {
...
#define within_range(x, L)  (-(1 << (L - 1)) < (x) && (x) < (1 << (L -1)) - 2)
      switch (howto->type)
        {
          ...
          case R_CKCORE_PLT32:
            ...
            if (h->plt.offset == (bfd_vma) -1
              ...
              {
                bfd_vma got_offset;
                if ((elf_elfheader (output_bfd)->e_flags & CSKY_ABI_MASK)
                    == CSKY_ABI_V2)
                  {
                    got_offset = (h->plt.offset / PLT_ENTRY_SIZE + 2) * 4;
                  }
                else
                  {
                    got_offset = (h->plt.offset / PLT_ENTRY_SIZE_P + 2) * 4;
                  }
                    ...
                    if (ELF32_R_TYPE (rel->r_info)
                        == R_CKCORE_PCREL_JSR_IMM26BY2)
                      {
                        if (within_range (disp, 26))
                          {
                            /* Change the data that in memory for preocessing,
                               and will not apply these to the object file.  */
                            howto =
                              &csky_elf_howto_table[R_CKCORE_PCREL_IMM26BY2];
                            read_content_substitute = CSKY_INSN_BSR32;
                          }
                          else if ((elf_elfheader (output_bfd)->e_flags
                                    & CSKY_ARCH_MASK) == CSKY_ARCH_810)
                          {
                             ...
                             == R_CKCORE_PCREL_JSR_IMM11BY2)
                      {
                        if (within_range (disp, 11))
                          {
                            /* Change the data that in memory for procssing,
                               and will not apply these to the object file.  */
                            howto =
                              &csky_elf_howto_table[R_CKCORE_PCREL_IMM11BY2];
          ...
          case R_CKCORE_ADDR_LO16:
            if (bfd_link_pic(info)
                                ...
                                || h->root.type == bfd_link_hash_indirect)))))
              {
                ...
                /* When geerating a shared object, these relocations
                 ...
                 if (skip)
                  {
                   ...
                   {
                     /* This aymbol is local, or marked to become local.  */
                     outrel.r_info = ELF32_R_INFO (0, R_CKCORE_RELATIVE);
                     outrel.r_addend = relocation + rel->r_addend;
                   }
          ...
          case R_CKCORE_TLS_LDM32:
              {
                ...
                if (off & 1)
                  ...
                  {
                    /* If we don't konw the module number,
          ...
          case R_CKCORE_TLS_LE32:
            if (bfd_link_dll (info))
              {
                (*_bfd_error_handler)
                  (_("%B(%A+0x%lx): R_CKCORE_TLS_LE32 relocation\
      ...
      /* Make sure a 32bits data in the text section will not be affected by
         out special endianess.
      ...
      if ((howto->type != R_CKCORE_PCREL_JSR_IMM11BY2)
          ...
          && (howto->type != R_CKCORE_NONE)
          && do_final_relocate)
        {
          r = _csky_final_link_relocate (howto, input_bfd, input_section,
                ...
                if (h != NULL)
                ...
                else
                  {
                    name = bfd_elf_string_from_elf_section (input_bfd,
                              symtab_hdr->sh_link, sym->st_name);
                    if (name == NULL)
                      {
                        break;
                      }
  ...
  switch (note->descsz)
    {
      default:
        return FALSE;
      /* Sizeof(struct elf_prstatus) on csky of CK610.  */
      case 148 :
       ...
       break;
     /* Sizeof(struct elf_prstatus) on csky of CK810.  */
     case 220 :
        elf_tdata (abfd)->core->signal
          = bfd_get_16 (abfd, note->descdata + 12);
